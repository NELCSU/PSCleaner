...
(function ChooseModelFolder() {
  const browse = one("#btnModelSelection");
  const clear = one("#btnClearModelSelection");
  const label = one("#lblModelFilePath");
  let folder, base;
  ...
  clear.addEventListener("click", () => {
    folder = null;
    ipc.send("delete-folder", "model");
    updatePathLabel();
  });
  
  function saveSelection() {
    if (folder) {
      ipc.send("save-folder", { key: "model", value: folder });
    }
  }
  ...
  ipc.on("response-folders", (e, resp) => {
    folder = fs.pathExistsSync(resp["model"]) ? resp["model"] : null;
    base = resp.default;
    updatePathLabel();
  });
})();
