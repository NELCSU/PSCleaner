const a: RegExp[] = [
  /\b\w{2,}\-\d{1,2}\b/gmi,
  /\b\d+\s(?:mile|kilometer|km|steps)s?\b/gmi,
  /\b(?:first|last)\s\d+\b/gmi,
  /\bat\shome\b/gmi,
  /\b(?:between|in)\s(?:April|Jan|June|March|May)\s(?:and|to)\s(?:April|Jan|June|March|May)\b/gmi,
  /\bthe\seve\sof\b/gmi,
  /\ba\sbath\b/gmi,
  /\bin\s(?:(?:early|mid|late)[\s\-])?(?:April|Jan|June|May)\b/gmi,
  /\bend\sof\s(?:April|Jan|June|May)\b/gmi,
  /\bhead\son\b/gmi,
  /\bmid\-\w+\b/gmi,
  /\bnext\sdoor\b/gmi,
  /\bone\sof\sthe\b/gmi,
  /\bon\stop\sof\b/gmi,
  /\b(?:from|on)\s(?:Fri|Mon|Satur|Sun|Thurs|Tues|Wednes)day\b/gmi,
  /\bsince\s(?:(?:early|mid|late)[\s\-])?(?:April|Jan|June|March|May)\b/gmi,
  /\bstepping\son\b/gmi,
  /\bwent\sover\b/gmi,

  /[a-z]+[\'\‘\’\`](?:ll|re|ve)/gmi,
  /[a-z]+n[\'\‘\’\`]t/gmi
];
const SkipGrammarRegEx: Set<RegExp> = new Set();
a.forEach(term => SkipGrammarRegEx.add(term));
export {
  SkipGrammarRegEx
};