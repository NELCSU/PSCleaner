...
export class NLP {
  ...
  public async evaluate(data: string): Promise<MatchedEntity[]> {
    ...
    if (namesEntity.enabled) {
      ...
      searches.push({
        action: { discard: 1, joinable: 1, order: 2, pos: 0, prefix: 0, midfix: 1, suffix: 1 },
        entity: namesEntity,
        matches: this._evalKeyword(data, (n: string) => isPropercase(n), ProperNameSetJoinMidfixSuffixOnly)
      });

      searches.push({
        action: { discard: 1, joinable: 1, order: 2, pos: 0, prefix: 0, midfix: 0, suffix: 1 },
        entity: namesEntity,
        matches: this._evalKeyword(data, (n: string) => isPropercase(n), ProperNameSetJoinSuffixOnly)
      });
